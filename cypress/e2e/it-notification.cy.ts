...
describe('IT unread notifications', () => {
  ...
  it('non-IT-staff role ไม่แสดง notification dropdown', () => {
  ...
  it('badge แสดงตัวเลขหรือ truncate เมื่อ unread มากกว่า 9 รายการ', () => {
    ...
    cy.get('.notification-dropdown .badge')
      .invoke('text')
      .invoke('trim')
      .should('match', /9\+|10/);
  });

  it('header "การแจ้งเตือนล่าสุด" แสดงเมื่อมี notification', () => {
    setupItNotificationApi([101, 202]);
  ...
  it('กด notification ticket 202 แล้ว redirect พร้อม focusZone=tickets', () => {
    setupItNotificationApi([202]);
    ...
    cy.wait('@markTicketRead').its('request.url').should('contain', '/tickets/202/read');
    cy.url().should('include', '/it-dashboard');
    cy.url().should('include', 'focusZone=tickets');
  });
});
