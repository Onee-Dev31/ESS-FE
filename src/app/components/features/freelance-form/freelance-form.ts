...
export class FreelanceFormComponent implements OnInit, OnChanges {
  ...
  onFileSelect(event: Event) {
    ...
    Array.from(input.files).forEach((file: File) => {
      ...
      if (!extension || !this.ALLOWED_EXTENSIONS.includes(extension)) {
        this.swalService.warning(`ไฟล์ ${file.name} ไม่รองรับประเภทนี้`);
        return;
      }

      // ❌ ขนาดเกิน
      if (file.size > this.MAX_SIZE) {
        this.swalService.warning(`ไฟล์ ${file.name} ต้องไม่เกิน 5MB`);
        return;
      }
  ...
  onMoneyInput(field: 'salary' | 'otherIncome', event: any) {
    ...
    const formattedInteger = integer ? parseInt(integer, 10).toLocaleString('en-US') : '';

    const formattedValue =
      limitedDecimal !== undefined ? `${formattedInteger}.${limitedDecimal}` : formattedInteger;
  ...
  private mapCompanyAndDepartment() {
    ...
    // 🔹 map company
    const selectedCompany = this.companyList.find((c) => c.COMPANY_CODE === this.formData.company);
  ...
  private syncMoneyDisplay() {
    this.salaryDisplay = this.formData.salary ? this.formatMoney(this.formData.salary) : '';
