...
export class VehicleTaxiFormV2Component implements OnInit, OnChanges {
  ...
  get claimStatus(): string {
    return String(this.requests?.status ?? '').trim();
  }

  get claimStatusLabel(): string {
    switch (this.claimStatus.toLowerCase().replaceAll('_', ' ')) {
      case 'new':
        return 'รายการใหม่';
      case 'pending':
        return 'รออนุมัติ';
  ...
  get confirmButtonLabel(): string {
    if (!this.isEditMode) return 'ยืนยันการเบิก';
    if (this.hasDelete()) return 'ลบรายการเบิกทั้งหมด';
    return this.isReferredBack ? 'ยืนยันการแก้ไขและส่งอนุมัติใหม่' : 'ยืนยันการแก้ไขการเบิก';
  }

  getEligibleDateLabel(date: TaxiLogItem): string {
    return `${this.dateUtil.formatDateToBE(date.date, 'ddd. DD/MM/YYYY')} — ${this.getEligibilityText(date)}`;
  }
  ...
  ngOnInit() {
    this.masterDataService.getDateConfig().subscribe((config) => {
      queueMicrotask(() => {
        ...
        this.years = Array.from({ length: 4 }, (_, index) => currentYear - index);
  ...
  save() {
    ...
    if (this.isEditMode && this.originalClaimId) {
      const claimId = this.originalClaimId;
      const isResubmit = this.isReferredBack;
      this.swalService
        .confirm(isResubmit ? 'ยืนยันการแก้ไขและส่งอนุมัติใหม่?' : 'ยืนยันการแก้ไขรายการเบิก?')
        .then((result) => {
          if (!result.isConfirmed) return;
          ...
          this.isSubmitting = true;
          this.taxiService
            .updateTaxiClaim(claimId, formData)
            .pipe(
              switchMap(() =>
                isResubmit
                  ? this.taxiService.reSubmitTaxiClaim({
                      claimId,
                      type: 'taxi',
                    })
                  : of(null),
            ...
            .subscribe({
              next: () => {
                this.swalService.success(
                  isResubmit
                    ? 'แก้ไขและส่งอนุมัติใหม่เรียบร้อยแล้ว'
                    : 'แก้ไขรายการเบิกเรียบร้อยแล้ว',
                );
                this.onClose.emit();
                return;
              ...
              error: (error) => {
                this.swalService.warning(this.getApiErrorMessage(error));
                return;
    ...
    if (!this.isEditMode && selectedItems.length === 0) {
      ...
      // Create
      this.swalService.confirm('ยืนยันการเบิก?').then((result) => {
        if (!result.isConfirmed) return;
        ...
        this.taxiService
          ...
          .subscribe({
            next: () => {
              this.swalService.success('สร้างรายการเบิกเรียบร้อยแล้ว');
