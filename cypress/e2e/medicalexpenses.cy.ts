...
describe('Medical Expenses', () => {
  ...
  it('pagination wrapper แสดงขึ้นในหน้า medicalexpenses', () => {
  ...
  it('modal form มีปุ่ม submit สำหรับยืนยันการเบิก', () => {
    cy.get('.btn-create').click();
    cy.get('app-medicalexpenses-form').should('be.visible');
    cy.get('app-medicalexpenses-form .btn-submit').should('exist');
  });

  it('กดปุ่ม info เปิด policy modal ได้', () => {
    cy.get('[title="ข้อมูลค่ารักษาพยาบาล/สวัสดิการ"]').click();
    cy.get('app-medical-policy-modal').should('be.visible');
  });

  it('stat-card รายการทั้งหมด แสดงตัวเลข', () => {
    cy.get('.stats-bar .stat-card').first().find('.stat-value').invoke('text').should('match', /\d+/);
  });
