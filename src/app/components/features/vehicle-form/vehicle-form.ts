...
export class VehicleFormComponent implements OnInit, OnChanges {
  ...
  onToggleCheck(log: VehicleLogItem) {
    if (log.selected) {
      const hasSelectedSameDate = this.logs.some(
        (item) =>
          item !== log &&
          item.selected &&
  ...
  onSubmit() {
    ...
    if (this.MODE_EDIT && selectedLogs.length === 0) {
      this.swalService.confirm('ยืนยันการลบรายการเบิกทั้งหมด').then((result) => {
        ...
        this.vehicleService
          .deleteVehicleByEmpCode(this.requests.id, this.authservice.userData().CODEMPID)
          .subscribe({
            next: (res) => {
              // console.log(res)
              if (!res?.success) {
    ...
    if (this.MODE_EDIT) {
      ...
      this.swalService.confirm('ยืนยันการแก้ไขการเบิก').then((result) => {
        ...
        this.vehicleService.updateVehicleByClaimId(this.requests.id, payload).subscribe({
          next: (res) => {
            // console.log(res)
            if (!res?.success) {
              this.swalService.warning('ไม่สามารถบันทึกข้อมูลได้');
    ...
    if (!this.MODE_EDIT && selectedLogs.length === 0) {
      ...
      this.swalService.confirm('ยืนยันการเบิก').then((result) => {
        ...
        this.vehicleService.createVehicleByEmpcode(payload).subscribe({
          next: (res) => {
            // console.log(res)
            if (!res?.success) {
              this.swalService.warning('ไม่สามารถบันทึกข้อมูลได้');
