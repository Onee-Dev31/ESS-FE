...
export class Sidebar {
  ...
  private allMenuItems: MenuItem[] = [
    ...
    },
    {
      ...
      iconType: 'fa',
      subItems: [{ label: 'คำขอลา', path: '/timeoff' }],
    },
    {
      name: 'อนุมัติ',
  ...
  menuItems = computed(() => {
    ...
    const formattedMenus = menus
      ...
      .map((parent: any) => {
        return {
          ...
          path:
            parent.RoutePath && parent.RoutePath.trim() !== ''
              ? parent.RoutePath.startsWith('/')
                ? parent.RoutePath
                : `/${parent.RoutePath}`
              : null,
          subItems: menus
    ...
    const mergedMenus = formattedMenus.reduce((acc: any[], current: any) => {
      ...
      if (existingMenuIndex !== -1) {
        ...
        if (
          (!existingMenu.path || existingMenu.path === '/') &&
          current.path &&
          current.path !== '/'
        ) {
        ...
        current.subItems.forEach((newSub: any) => {
          const existingSubIndex = existingMenu.subItems.findIndex(
            (sub: any) => sub.label === newSub.label,
          );
          if (existingSubIndex === -1) {
            ...
            if (
              (!existingSub.path || existingSub.path === '/') &&
              newSub.path &&
              newSub.path !== '/'
            ) {
