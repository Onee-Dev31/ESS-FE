...
export function createListingComputeds_v2<T>(data: Signal<T[]> | (() => T[]), state: ListingState) {
  const sourceData: () => T[] = isSignal(data) ? data : (data as () => T[]);
  ...
  const startIndex = computed(() =>
    totalItems() === 0 ? 0 : state.currentPage() * state.pageSize() + 1,
  );

  const endIndex = computed(() => state.currentPage() * state.pageSize() + sourceData().length);
